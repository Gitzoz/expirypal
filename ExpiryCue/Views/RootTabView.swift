...
private struct ExpiryCueScreenModifier: ViewModifier {
    func body(content: Content) -> some View {
        content
            .scrollContentBackground(.hidden)
            .background(AppTheme.background.ignoresSafeArea())
            .toolbarBackground(AppTheme.sandStrong, for: .navigationBar)
            .toolbarBackground(.visible, for: .navigationBar)
            .toolbarColorScheme(.light, for: .navigationBar)
    }
}

private struct ExpiryCueCardModifier: ViewModifier {
    let fill: Color
    let border: Color
...
extension View {
    func expiryPalScreenChrome() -> some View {
        modifier(ExpiryCueScreenModifier())
    }

    func expiryPalCard(fill: Color = AppTheme.sandStrong, border: Color = AppTheme.border) -> some View {
        modifier(ExpiryCueCardModifier(fill: fill, border: border))
    }

    func expiryCueNavigationTitle(_ titleKey: LocalizedStringKey, isScreenshotMode: Bool = false) -> some View {
            ...
            .toolbar {
                ToolbarItem(placement: .principal) {
                    Text(titleKey)
                        ...
                        .shadow(color: AppTheme.sand.opacity(0.7), radius: 0.5, y: 0.5)
                        .accessibilityIdentifier(isScreenshotMode ? "screenshot.mode.title" : "navigation.title")
                }
            }
    }
}
...
struct ExpiryCuePill<Label: View>: View {
    let fill: Color
