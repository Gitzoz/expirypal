...
final class LocalizationParityTests: XCTestCase {
    ...
    private func loadStrings(languageCode: String) throws -> [String: String] {
        let url = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
            .appendingPathComponent("ExpiryCue/Resources/\(languageCode).lproj/Localizable.strings")
    ...
    func testScreenshotModeUsesInMemoryStoreAndParsesScene() {
        let configuration = AppLaunchConfiguration.from(
            arguments: ["ExpiryCue", "SCREENSHOT_MODE", "SCREENSHOT_SCENE=editItem"]
        )
    ...
    func testUITestModeUsesInMemoryStoreWithoutScreenshotScene() {
        let configuration = AppLaunchConfiguration.from(
            arguments: ["ExpiryCue", "UITEST_MODE"]
        )
